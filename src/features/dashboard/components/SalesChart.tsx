    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <p className="text-xs text-text-muted">Sales performance over last 7 days</p>
        </div>
        <div className="flex gap-2">
          {['This Week', '7 Days', '30 Days'].map((t, i) => (
            <button 
              key={t}
              className={cn(
                "px-4 py-1.5 rounded-lg text-xs font-semibold border transition-all",
                i === 0 ? "bg-white border-border-light shadow-sm text-text-dark" : "bg-g-faint border-transparent text-text-mid hover:bg-g-pale"
              )}
            >
              {t}
            </button>
      ...
      <CardContent>
        ...
        <div className="flex items-center justify-between mt-6 pt-6 border-t border-border-light">
          <div className="flex items-baseline gap-4">
            <span className="text-xl font-bold text-text-dark">£4,230</span>
            <span className="text-xs text-text-muted">last 7 days</span>
            <span className="text-lg font-bold text-text-dark ml-4">£178</span>
